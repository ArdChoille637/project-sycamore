...
| `05_cfd/lbm3d_flow.py` | 3-D obstacle flow on the core (solid bounce-back + velocity inlet + outlet + momentum-exchange force) — the infrastructure toward the rotating samara. Flow validated on a sphere (clean recirculation wake); absolute force uncalibrated (same factor as the 2-D MEM). |
| `05_cfd/lbm3d_rotor.py` | **Rotating-reference-frame 3-D LBM** — Coriolis + centrifugal via Guo forcing (the implicit u↔force coupling solved analytically). Validated on **solid-body rotation** (fluid stays at rest in-frame, |u|<2e-3; analytic density profile `ρ₀·e^{1.5Ω²r²}` to 4.8%). The **revolving-samara-wing demo** shows the leading-edge vortex + the spanwise (radial) drainage flow a 2-D static section can't sustain (early-time, Mach-limited, qualitative). |
| `05_cfd/lbm3d_plume.py` | **3-D buoyant fire plume** ("the 4-D fire") — a transported temperature scalar + a Guo **Boussinesq buoyancy** body force on the core. A held fire patch drives a convective column that an ambient wind bends over — the fire–atmosphere coupling (WRF-SFIRE in miniature) that lofts smoke and drives indrafts. Buoyancy force validated analytically: a uniform force gives `w = β·t` to **0.0%**. |
...
| `02_fire/fire_coupling.py` | **Fire–atmosphere coupling** (the WRF-SFIRE feedback, "a fire makes its own wind") — the fire's own buoyant indraft fed back into the Rothermel wind. A fire is a mass **sink** in the *same* WindNinja Poisson solve (continuity `∇·V=−S`), giving a mass-consistent convergent surface wind, Picard-iterated with the CA to a fixed point. The indraft is **calibrated to the resolved 3-D LBM plume** (peak ≈ 0.73× ambient). Validated: inward-sign + mass conservation + a no-indraft null control + boundary-isolation guard. Result (weak wind): the steady kinematic indraft **retards the fire — flanks most** (crosswind width −45%, head −18%) via lateral convergence; *not* the unsteady pyroconvective head *acceleration* (that is plume-momentum, out of scope). |
